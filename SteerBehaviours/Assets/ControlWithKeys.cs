...
public class ControlWithKeys : MonoBehaviour {
	...
	void Update () {
        ...
        //transform.Rotate(0f,0f, Input.GetAxis("Horizontal") * Time.deltaTime * 100);// on turn give the car a realistic 'falling over' effect. 
        transform.Rotate(0f, Input.GetAxis("Horizontal") * Time.deltaTime * 130, 0f);//rotate when 'a'/'d'/'LEFTKEY'/'RIGHTKEY' is pressed to make it look as if it's turning. 
